...
_messageController.addListener(_onEdited);
  ...
  Future<void> _loadAboutData() async {
    ...
    _titleController.text =
        (data['aboutTitle'] ?? '用心照顧每一隻貓咪，讓牠們在這裡安心生活。')
            .toString();
  ...
  try {
    ...
    if (bytes.length > _maxImageBytes) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('圖片需小於 5MB')),
      );
      return;
    }
...
'updatedAt': FieldValue.serverTimestamp(),
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(content: Text('關於我們已儲存')),
  );
}
    ...
    children: [
      ...
      SizedBox(
        ...
        child: OutlinedButton.icon(
          onPressed: _uploadingImage ? null : _pickAndUploadAboutImage,
    ...
    child: TextButton.icon(
      onPressed: () async {
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('封面圖已刪除')),
          );
        } catch (e) {
          if (!mounted) return;
